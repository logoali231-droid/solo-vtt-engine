...
const INJECTION_RULES: Rule[] = [
  ...
  {
    family: "injection",
    re: /\bignore (all |any |the |your |previous |everything |my |all the |all previous |any previous |the previous )?(instructions|rules|prompts?|directives|orders|system)\b/i,
    en: "The GM's instructions and the rules engine are fixed — no message can override them. Describe what your character actually does and the dice will decide.",
    pt: "As instruções do GM e o motor de regras são fixos — nenhuma mensagem pode sobrescrevê-los. Descreva o que seu personagem realmente faz e os dados decidirão.",
  },
  {
...
const GRANT_RULES: Rule[] = [
  ...
  {
    family: "grant",
    re: /\b(gain|earn|get|receive|grant me) (a |an |the |free |instant |max |maximum |all |every )?(level|feat|spell|skill|proficien\w+|stat point|attribute point|item|weapon|armor|class feature)\b/i,
...
const OUTCOME_RULES: Rule[] = [
  ...
  {
    family: "declared-outcome",
    re: /(conte|conta|considere|considera|trate|trata|tome|tomar) (a |aquela |essa |esse |este |minha |o |meu )?(rolagem|resultado|teste|dado|falha|erro|check|checagem|ataque|salvamento) (como |por |para |pra |um |uma |como um |como uma )?(um |uma )?(sucesso|crit|crítico|critico|crítica|critica|passar|acerto|acertou|passou|vitória|vitoria|falha|fumble|fracasso)/i,
    en: "Declared outcomes don't change the dice — the engine resolves what actually happens.",
    pt: "Resultados declarados não mudam os dados — o motor resolve o que realmente acontece.",
  },
];
...
const RIGGING_RULES: Rule[] = [
  ...
  {
    family: "rigging",
    re: /\b(make|set|turn) (it|the roll|the result|the outcome) (a |an |to |into |nat |natural )?(nat ?20|natural 20|20|crit|critical|success|pass)\b/i,
  ...
  {
    family: "rigging",
    re: /\broll (a |an |the )?(nat ?20|natural 20|20|crit|critical) (for|to) (me|us)\b|\broll (me|us) (a |an |the )?(nat ?20|natural 20|20|crit|critical)\b/i,
    en: "The engine rolls — asking it to roll a specific result is rigging. Describe the action instead.",
    pt: "O motor rola — pedir para rolar um resultado específico é trapaça. Descreva a ação.",
  },
  {
    family: "rigging",
    re: /\brola (pra |para |pro |p\/ |mim |eu )?(um |uma )?(nat ?20|20|crit|crítico|critico|crítica|critica|sucesso|passar)\b/i,
    en: "The engine rolls — asking it to roll a specific result is rigging. Describe the action instead.",
    pt: "O motor rola — pedir para rolar um resultado específico é trapaça. Descreva a ação.",
  },
  {
...
const AUTHORITY_RULES: Rule[] = [
  ...
  {
    family: "authority",
    re: /\bcomo (mestre|mestra|gm|dm|narrador|deus) (eu|nós|a gente) (decido|decidimos|digo|dizemos|mando|mandamos|quero|queremos|vou|vamos) (que |pra |para |q |que eu )?(ganho|ganhamos|venço|vencemos|passo|passamos|não falho|nao falho|sucesso|vitória|vitoria)\b|\b(sou|serei) (o |a )?(mestre|mestra|gm|dm|narrador) (e |, )?(eu )?(decido|digo|quero|vou|mando) (que |q )?(eu )?(ganho|venço|passo|não falho|nao falho)\b/i,
    en: "The engine is the GM — no role claim changes a mechanic.",
    pt: "O motor é o mestre — nenhuma declaração de papel muda uma mecânica.",
  },
];
